...
폰에서 쓰는 걸 기준으로 만든 앱형 UI입니다. 하단 탭바 + 우측 하단 추가 버튼이라
한 손으로 엄지만 써도 대부분의 조작이 됩니다.
...
| 동작 | 결과 |
|---|---|
| 항목 체크 | 완료 처리 (체크 그려지는 애니메이션 + 진동 피드백) |
| 항목 탭 | 수정 시트 열기 |
| 왼쪽으로 밀기 | 삭제 — **실행취소** 버튼이 뜨는 동안 되돌릴 수 있음 |
| 아래로 당기기 | 새로고침 |
| 우측 하단 ➕ | 지금 보고 있는 탭에 맞춰 추가 시트 열기 |
| 우측 상단 ☾ / 🔔 | 다크모드 전환 / 요약 알림 즉시 발송 |

- **입력 최소화**: 추가 시트에서 `오늘 / 내일 / 다음 주`, `매일 / 매주 / 매월`,
  자주 사는 품목(우유·계란·휴지…)을 칩으로 바로 고를 수 있습니다.
- **모션**: 카드 순차 등장, 진행률 링 채우기, 탭 전환, 바텀 시트 슬라이드 등.
...
히어로는 밝은/어두운 테마 모두에서 잉크색 블록으로 고정해 브랜드 면을 만듭니다.
...
│       ├── index.html  앱 셸 (아이콘 스프라이트 · 히어로 · 탭바 · 바텀시트)
