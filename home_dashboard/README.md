...
| 메뉴 | 내용 |
|---|---|
| 오늘 | 시간대 배너 + 지난 할일 / 오늘 할일 / 오늘·다가오는 일정 / 살 것 |
...
오늘 화면 맨 위 배너는 **시각에 따라 하늘 색이 바뀝니다** (새벽 · 아침 · 낮 · 저녁 · 밤).
그 위에 도트 고양이 나비가 앉아 있고, 사이드바에도 작게 한 마리 있습니다.

| 상황 | 나비의 반응 |
|---|---|
| 평소 | 숨 쉬듯 1픽셀 위아래로 움직이고 3~6초마다 눈을 깜빡임 |
| 할일 완료 | 폴짝 뛰며 눈웃음, 체크한 자리에서 도트 색종이가 튀어오름 |
| 지난 할일이 있음 | 눈이 처지며 시무룩 |
| 오늘 할일을 다 끝냄 | 계속 기뻐하는 표정 |
| 밤 (21시~5시) | 눈 감고 Z 를 띄우며 잠 |

**이미지 파일은 한 장도 쓰지 않습니다.** 16×16 픽셀 배열을 캔버스에 찍는 방식이라
용량이 코드 몇 KB뿐이고, 확대해도 깨지지 않으며 팔레트만 바꾸면 색도 따라옵니다.
그림은 폰 브라우저가 그리므로 라즈베리파이에는 부하가 없고, 다른 앱을 보는 동안에는
브라우저가 애니메이션을 자동으로 멈춥니다. "동작 줄이기"를 켠 기기에서는 정지 그림만 나옵니다.

캐릭터 이름·모양은 `app/static/pixel.js` 맨 위의 픽셀 배열과 `PET_NAME` 에서 바꿉니다.
...
| 축 | 정한 것 |
|---|---|
| 색 | **크림 & 테라코타** — 종이 같은 크림 바탕(`#faf7f2`)에 구운 흙색 포인트(`#c2622d`). 다크는 회색 대신 코코아 톤 |
| 구조 | 표면 4단계(`--bg`·`--surface`·`--surface-2/3`) + 글자 3단계(`--ink`·`--ink-2`·`--ink-3`) + 강조 1색 |
...
| 도트 | 캐릭터·색종이는 16×16 픽셀 배열 → 캔버스 (`pixel.js`). 기능 아이콘은 선화 SVG 유지 |
...
│       ├── pixel.js    도트 캐릭터 스프라이트 · 애니메이션 · 완료 이펙트
